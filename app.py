...
st.markdown(
    ...
    ">
        <span style="font-weight: bold; font-size: 1.3em; color: #1e4620;">📊 결과</span><br>
        <div style="color: #212529; line-height: 2.0; font-size: 16px; margin-top: 8px;">
            •&nbsp;&nbsp;강원도 외국인 관광객의 쇼핑 소비는 3년 연속 기타관광쇼핑에 64% 이상 집중되어 있어 관광 관련 소매·기념품 소비가 쇼핑 지출의 핵심인 것으로 나타났다.
        </div>
    </div>
    """,
    unsafe_allow_html=True
...
st.markdown(
    ...
    ">
        ...
        <div style="line-height: 2.0; margin-top: 8px;">
            ...
            <span style="color: #212529; font-size: 16px;">
                •&nbsp;&nbsp;특히 기타관광쇼핑이 전체 쇼핑 소비의 약 65%를 차지하고 있는 만큼, 관광지 인근 상점과 특산품 판매장의 경쟁력 강화가 관광 소비 증대에 중요한 역할을 할 것으로 판단된다.
            </span>
        </div>
    </div>
    """,
...
# 5. 안내 및 인사이트 박스 레이아웃 (5번 섹션과 💻사용한 SQL 파트 디자인/하이라이팅 100% 일치 버전)
with st.expander("💻 사용한 SQL"):
    st.markdown("##### 👚 패션 콘텐츠 경로 조회 전체 쿼리")
    st.code(
        """SELECT 
    SUM(CASE WHEN IQ2_1 = 0 THEN 1 ELSE 0 END) AS "소셜네트워크서비스",
    SUM(CASE WHEN IQ2_2 = 0 THEN 1 ELSE 0 END) AS "드라마, 예능, 영화",
...
WHERE SQ1a = 1 OR SQ1a = 13;""", 
    ...
    st.markdown("##### 💄 뷰티 콘텐츠 경로 조회 전체 쿼리")
    st.code(
        """SELECT 
    SUM(CASE WHEN JQ2_1 = 0 THEN 1 ELSE 0 END) AS "소셜네트워크서비스",
    SUM(CASE WHEN JQ2_2 = 0 THEN 1 ELSE 0 END) AS "드라마, 예능, 영화",
...
st.markdown(
    ...
    ">
        <span style="font-weight: bold; font-size: 1.3em; color: #1e4620;">📊 결과</span><br>
        <div style="color: #212529; line-height: 2.0; font-size: 16px; margin-top: 8px;">
            • 미국인과 중국인이 가장 많이 소비하는 한류 콘텐츠인 뷰티·패션 콘텐츠의 주요 유입 경로를 분석한 결과, 두 콘텐츠 모두 유튜브와 자국 내 뷰티·패션 쇼 및 전시회가 핵심 접점으로 나타난다.<br>
            • 다양한 경로를 통해 콘텐츠가 유입되고 있으나 특정 매체에 집중되기보다 여러 채널을 복합적으로 활용하여 한류 콘텐츠를 접하는 경향이 확인된다.
        </div>
    </div>
    """,
    unsafe_allow_html=True
...
st.markdown(
    ...
    ">
        ...
        <div style="color: #212529; line-height: 2.0; font-size: 16px; margin-top: 8px;">
            • <span style="font-weight: 800; color: #000000;">미국인과 중국인은 한류 뷰티·패션 콘텐츠를 주로 유튜브를 통해 접하고 있어, 강원도 관광·축제 홍보 역시 유튜브를 중심으로 홍보를 추진할 필요가 있다.</span><br>
