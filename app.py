...
# 2. [패션 그래프] 상위 3개 색상 추출 및 맵 생성 (조금 더 진하고 선명한 컬러)
df_fashion_sorted = df_fashion_melted.sort_values(by="빈도수", ascending=False)
# 🎨 그래프 조각용 진한 인디 컬러 (코랄 핑크, 에메랄드 그린, 딥 옐로우)
strong_colors = ['#ff6b81', '#2ed573', '#ffa502'] 
# ✍️ 글씨용 선명한 대비 컬러 (핫핑크, 진한 초록, 진한 주황)
...
for i, (idx, row) in enumerate(df_fashion_sorted.iterrows()):
    ...
    if i < 3:
        ...
        fashion_color_map[path_name] = '#e0e0e0'  # 일반 회색 조각도 약간 더 진하게
...
with col1:
    ...
    for path in df_fashion_melted['접근경로']:
        # 실시간 데이터 순위를 찾아서 1, 2, 3위에 따라 선명한 글씨색 부여
        rank = list(df_fashion_sorted['접근경로']).index(path)
        if rank < 3:
            fashion_text_colors.append(text_strong_colors[rank]) # 쨍한 포인트 글씨색
            fashion_text_sizes.append(15)                        # 글씨 크기 15 (크게)
        else:
            fashion_text_colors.append('#222222')                # 하위 항목은 선명한 검은색
    ...
    fig1.update_traces(
        ...
        textfont=dict(
            ...
            family="Malgun Gothic, Apple SD Gothic Neo, sans-serif" # 글꼴 가독성 확보
        ),
        marker=dict(line=dict(color='#ffffff', width=2.0)) # 흰색 구분선도 조금 더 두껍게(2.0)
    )
    fig1.update_layout(
        title_font_size=18, 
        showlegend=False,
        margin=dict(t=50, b=50, l=70, r=70) # 글씨 짤림 방지 마진 확대
    )
...
with col2:
    ...
    for path in df_beauty_melted['접근경로']:
        ...
        if rank < 3:
            beauty_text_colors.append(text_strong_colors[rank])  # 쨍한 포인트 글씨색
            beauty_text_sizes.append(15)
        else:
            beauty_text_colors.append('#222222')                 # 하위 항목은 선명한 검은색
